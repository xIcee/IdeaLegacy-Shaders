...
// bump mapping shader with parallax + specular
// requires terrain_nh.png and terrain_s.png
...
#ifdef WAVING_WATER
// excludes pistons and living entities
...
//if (tex == 14) {
    ...
    if ((mc_Entity.x == -1 && gl_Color.a == 1.0)
        ...
        && squareTex == 0)
    {   // entities with non-square textures
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            vertColor += ls.diffuse * max (dot (normalFix, posFix), 0.0) * gl_Color;    //gl_FrontMaterial.diffuse;
             ...
             && !(tex >= 106 && tex <= 110))
    {   // pistons etc
        vec3 normalFix = gl_Normal * gl_NormalMatrix;
        vertColor *= gl_LightModel.ambient;
        for (int i = 0; i < 2; ++i)
        {
            vec3 posFix = (gl_LightSource[i].position).xyz;
    ...
    viewVectorSpec = normalize (tbnMatrix * cameraPosition);
    ...
    viewVector = normalize (tbnNormMatrix * viewVector);
